...
class authen : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val GuestSignInButton  = view.findViewById(R.id.button2) as Button
        ...
        button3.setOnClickListener{
            ...
            builder.setMessage("คุณชอบ Application เราหรือไม่?")

            builder.setNegativeButton("ชอบ",
                DialogInterface.OnClickListener{ dialog, id ->
                    Toast.makeText(this.context,"ขอบคุณครับ", Toast.LENGTH_SHORT).show()
                    dialog.dismiss()
                })

            builder.setPositiveButton("ไม่ชอบ",
                DialogInterface.OnClickListener{ dialog, id ->
                    Toast.makeText(this.context,"ขออภัยในความไม่สะดวก", Toast.LENGTH_SHORT).show()
                    dialog.dismiss()
                })
            builder.show();
        }
